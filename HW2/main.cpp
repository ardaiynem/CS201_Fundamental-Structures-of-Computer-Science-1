...
int main() {
    time_point <system_clock> startTime_Naive;
    time_point <system_clock> startTime_NaiveCycle;
    time_point <system_clock> startTime_Recursive;

    duration <double, milli> elapsedTime_Naive;
    duration <double, milli> elapsedTime_NaiveCycle;
    duration <double, milli> elapsedTime_Recursive;
    ...
    for (int i = 0; i < P_SIZE; ++i) {
        for (int j = 0; j < N_SIZE ; ++j) {
            ...
            startTime_NaiveCycle = system_clock::now();
            for (int k = 0; k < 1000; ++k) {
                naiveAlgorithm_cycleShortcut(a, arr_n[j], arr_p[i]);
            }
            elapsedTime_NaiveCycle = system_clock::now() - startTime_NaiveCycle;
    ...
    for (int i = 0; i < P_SIZE; ++i) {
        for (int j = 0; j < N_SIZE ; ++j) {
            ...
            startTime_Recursive = system_clock::now();
            for (int k = 0; k < 1000000; ++k) {
                recursiveAlgorithm(a, arr_n[j], arr_p[i]);
            }
            elapsedTime_Recursive = system_clock::now() - startTime_Recursive;
    ...
    for (int i = 0; i < P_SIZE; ++i) {
        for (int j = 0; j < N_SIZE ; ++j) {
            ...
            startTime_Naive = system_clock::now();
            naiveAlgorithm(a, arr_n[j], arr_p[i]);
            elapsedTime_Naive = system_clock::now() - startTime_Naive;
